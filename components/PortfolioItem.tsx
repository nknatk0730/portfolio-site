...
export const PortfolioItem = ({portfolio}: {portfolio: (typeof portfolios)[0]}) => {
        ...
        {portfolio.images.map((image, index) => (
          ...
          >
            <Image
              ...
              className="object-cover"
            />
            <div className={`bg-black bg-opacity-10 backdrop-blur-[1px] absolute top-0 left-0 w-full h-full transition-opacity duration-300 ${currentImg !== image ? 'opacity-100' : 'opacity-0'}`}></div>
          </div>
        ))}
      </div>
    </div>
